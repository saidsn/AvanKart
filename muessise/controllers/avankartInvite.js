...
export const invitePartner = async (req, res) => {
  ...
  const myUser = await PartnerUser.findById(userId).populate('muessise_id');
  const senderMuessiseId = myUser.muessise_id._id;
  ...
  // Check muessise_id for single ID
  if (id) {
    ...
    const idquery = isObjectId
                        ...
                        : { partnyor_id: id };
    const receiverUser = await PartnerUser.findOne(idquery);
    if (!receiverUser) {
      return res
        .status(404)
        .json({ message: res.__("messages.invite.receiver_user_not_found") });
    }
    if(receiverUser.muessise_id){
      if (receiverUser.muessise_id.toString() === senderMuessiseId.toString()) {
        return res.status(400).json({
          message: res.__("messages.invite.cannot_invite_same_muessise"),
        });
      }
    }
  }

  // Check muessise_id for multiple IDs
  if (ids && ids.length > 0) {
    const objectIds = ids.filter(id => mongoose.Types.ObjectId.isValid(id));
    const stringIds = ids.filter(id => !mongoose.Types.ObjectId.isValid(id));

    const receiverUsers = await PartnerUser.find({
                                        $or: [
                                          { _id: { $in: objectIds } },
                                          { partnyor_id: { $in: stringIds } }
                                        ]
                                      });
    const invalidUsers = receiverUsers.filter(
      (user) =>
        user.muessise_id &&
        senderMuessiseId &&
        user.muessise_id.toString() === senderMuessiseId.toString()
    );
  ...
  const cancelPreviousPending = async (userId) => {
    await InvitePartner.updateMany(
  ...
  try {
    ...
    const inviteResults = await InvitePartner.insertMany(invites);
    ...
    await Promise.all(
      inviteResults.map(async (inviteResult, index) => {
        await NotificationModel.create({
          ...
          type: "invation",
          category: "corporate",
          title: myUser.muessise_id?.muessise_name ?? "Yeni dəvət",
          muessise_id: myUser.muessise_id?._id,
    ...
    await sendFirebaseNotificationToUsers(
      invites.map((invite) => invite.user_id),
      {
        ...
        data: {
          ...
          muessise_id: senderMuessiseId.toString(),
        },
      }
    );
