import TempUserOtpService from "../../../shared/model/partner/tempUserOtpService.js";
import PartnerUser from "../../../shared/models/partnyorUserModel.js";
import { generateOtp, sendMail, sendSms } from "../../../shared/utils/otpHandler.js";
...
export const changeOtp = async (req, res) => {
  ...
  try {
    ...
    if (changed === "authenticator") {
      ...
      if (now - lastRequest < 1000) {
        const existingOtp = await TempUserOtpService.findOne({ user_id: user._id, changed })
          .sort({ createdAt: -1 });
        if (existingOtp) {
      ...
      // TempUserOtpService-də saxla
      const otpDoc = new TempUserOtpService({
        user_id: user._id,
    ...
    const otp = generateOtp().toString().trim(); // String saxla

  if (changed === "email") {
  try {
    await sendMail(user.email, otp, debug);

    // 🔹 Debug rejimində OTP konsola yazılsın
    if (debug) {
      console.log(`[DEBUG][EMAIL OTP] ${user.email} üçün OTP: ${otp}`);
    }
  ...
  try {
    ...
    if (debug) {
      console.log(`[DEBUG][SMS OTP] ${user.phone_suffix}${user.phone} üçün OTP: ${otp}`);
    }
    ...
    return res.status(500).json({
      success: false,
      message: "SMS göndərilmədi"
    });
  }
}


    // 🔹 Əvvəlki OTP-ləri sil (çox OTP qalmasın deyə)
    await TempUserOtpService.deleteMany({ user_id: user._id, changed });

    // 🔹 Yeni OTP-ni DB-yə yaz
    await new TempUserOtpService({
      user_id: user._id,
      otp_code: otp, // String olaraq saxlanır
      changed,
      accepted_at: null
    }).save();
...
export const acceptOtpServices = async (req, res) => {
  try {
    ...
    const latestRequest = await TempUserOtpService.findOne({
      user_id: userId,
      accepted_at: null,
    }).sort({ createdAt: -1 });
    ...
    await TempUserOtpService.findByIdAndUpdate(latestRequest._id, {
    ...
    await PartnerUser.findByIdAndUpdate(userId, { $set: updateFields });
