...
export const muessiseMuqavile = async (req, res) => {
  ...
  try {
    const userFromDb = await PartnerUser.findById(user.id).select('muessise_id');
    if (!userFromDb) {
      return res.status(404).json({ error: "User not found" });
    }

    const baseQuery = {
      muessise_id: userFromDb.muessise_id,
    };
...
export const filterMuessiseMuqavile = async (req, res) => {
  ...
  try {
    ...
    const baseQuery = {
      $or: [
        { muessise_id: userFromDb.muessise_id } // köhnə model üçün fallback
      ]
    };

    console.log("bura geldir",req.csrfToken(),"date",start_date)
    const filterQuery = { ...baseQuery };
