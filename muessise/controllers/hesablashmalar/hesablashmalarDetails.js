...
export const details = async (req, res) => {
  try {
    ...
    console.log("Found transactions for hesablasma:", transactions.length);
    ...
    const cardBreakdownWithPercentage = Object.values(cardStats).map((card) => {
      const percentage =
        totalNetAmount > 0 ? (card.totalNetAmount / totalNetAmount) * 100 : 0;
      return {
        ...card,
    ...
    const hesablasmaData = {
      ...
      date: hesablasma.createdAt
        ? new Date(hesablasma.createdAt).toLocaleDateString("az-AZ")
        : new Date().toLocaleDateString("az-AZ"),
...
export const detailsDataTable = async (req, res) => {
  try {
    ...
    console.log("detailsDataTable called with:", {
      invoiceId,
      draw,
      start,
      length,
    ...
    if (search && search.value && search.value.trim() !== "") {
      console.log("Search query:", search.value.trim());
    }

    // Hesablaşma məlumatlarını tap
    let hesablasma = await Hesablasma.findOne({ hesablasma_id: invoiceId });
    console.log("Found hesablasma:", hesablasma);
    ...
    console.log("Amount filters:", { min_amount, max_amount });
    ...
    const aggregatePipeline = [
      {
        $match: {
          hesablasma_id: hesablasma._id,
          deleted: { $ne: true }, // Soft delete check
      ...
      // 🔥 Amount range filtri
      {
        $match: {
          ...
          min_amount !== null &&
          min_amount !== "" &&
          !isNaN(parseFloat(min_amount))
            ? { netAmount: { $gte: parseFloat(min_amount) } }
            : {}),
          ...(max_amount !== undefined &&
          max_amount !== null &&
          max_amount !== "" &&
          !isNaN(parseFloat(max_amount))
            ? { netAmount: { $lte: parseFloat(max_amount) } }
            : {}),
        },
      },

      // 🔥 Search filtri - улучшенный поиск
        ...
        ? [
            {
              $match: {
                $or: [
                  ...
                  // Поиск по сумме (как число)
                  ...(isNaN(parseFloat(search.value.trim()))
                    ? []
                    : [
                        {
                          amount: parseFloat(search.value.trim()),
                        },
                      ]),
    ...
    // Fallback: if no results and we have filters, try without filters to get total count
    if (
      recordsFiltered === 0 &&
      (cards.length > 0 ||
        min_amount ||
      ...
      console.log(
        "No results with filters, checking total count without filters..."
      );
    }
