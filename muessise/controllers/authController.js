...
import { extractDeviceInfo, generateFingerprint } from "../utils/fingerprint.js";
...
export const logout = async (req, res) => {
  ...
  if (sessionId) {
    ...
    try {
      if (userId) {
        await Session.deleteMany({ user_id: new mongoose.Types.ObjectId(userId) });
        console.log("✅ MongoDB sessions deleted for user:", userId);
      }
    } catch (err) {
...
export const loginPost = async (req, res) => {
  try {
    ...
    const mongoResult = await Session.findOneAndUpdate(
      { 
        user_id: user._id, 
    ...
    const testQuery = await Session.findOne({
      user_id: user._id,
      device_name: deviceInfo.userAgent,
...
export const forgetPasswordPost = async (req, res) => {
  ...
  try {
    const user = await PartnerUser.findOne({ email }).select(
    ...
    try {
      ...
      await Session.findOneAndUpdate(sessionQuery, updateData, {
        upsert: true,
        new: true,
      });
    } catch (err) {
...
export const otpVerifyPost = async (req, res) => {
  ...
  const user = await PartnerUser.findById(userId.id);
  ...
  // 🔹 Session update
  const deviceOsRaw = req.headers["sec-ch-ua-platform"] || "Unknown";
  const deviceOs = deviceOsRaw.replace(/^"|"$/g, "");
