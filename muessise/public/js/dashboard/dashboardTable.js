$(document).ready(function () {
  ...
  const useServerSide = true; // Server-side processing flag
  let table;
    ...
    window.dataTable = $("#myTable").DataTable({
      ...
      ajax: {
        ...
        dataSrc: function (json) {
          ...
          window.cardsArray = json.mycards;
          if (json.error) {
  ...
  $("#customSearch").on("keyup", function () {
    ...
    if (window.dataTable) {
      window.dataTable.search(searchValue).draw();
      if (!useServerSide) {
  ...
  window.changePage = function (page) {
    if (window.dataTable) {
      var pageInfo = window.dataTable.page.info();
      if (page < 0) page = 0;
      if (page >= pageInfo.pages) page = pageInfo.pages - 1;
      window.dataTable.page(page).draw("page");
    }
  };
  ...
  window.applyFilters = function () {
    ...
    if (useServerSide && window.dataTable && window.dataTable.ajax) {
