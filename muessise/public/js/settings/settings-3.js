...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const timeoutModal = document.getElementById("timeoutModal");
  const startBtn = document.getElementById("startOtpTimerBtn");
  const retryBtn = document.querySelector("#timeoutModal .bg-primary"); // "Yenidən yoxla" düyməsi

  if (!countdownEl || !timeoutModal || !startBtn || !retryBtn) return;

  let interval;

  function startTimer() {
    let time = 30; // 30 saniyə
    countdownEl.textContent = `00:${time < 10 ? '0' + time : time}`;

    // Əgər interval əvvəldən işləyirsə dayandır
    ...
    interval = setInterval(() => {
      ...
      if (time <= 0) {
        ...
        timeoutModal.classList.remove("hidden"); // Timeout modal aç
      }
    }, 1000);
  }

  // Başla düyməsinə basanda timer
  startBtn.addEventListener('click', () => {
    startTimer();
  });

  // Yenidən yoxla düyməsinə basanda timer yenidən işləsin
  retryBtn.addEventListener('click', () => {
    timeoutModal.classList.add("hidden"); // modalı gizlət
    startTimer(); // timer-i yenidən başlat
  });
...
let toggleclick = false;
  ...
  function clearOtpInputs(formId) {
    const otpInputs = document.querySelectorAll(`#${formId} .otp-input`);
...
function toggleEmail(formName) {
  ...
  if (toggleclick) {
    clearOtpInputs("toggleEmailOtpForm");
...
function toggleNomre(formName) {
  ...
  clearOtpInputs("toggleNomreOtpForm");
  submitForm(formName);
...
function closeVaxtBitdi() {
  const timeoutModal = document.getElementById("timeoutModal");
  const otp = document.querySelector(".authenticator-tesdiq");
  if (timeoutModal) {
    timeoutModal.classList.add("hidden");
