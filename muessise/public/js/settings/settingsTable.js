$(document).ready(function () {
  ...
  // HTML Escape funksiyası - XSS hücumlarının qarşısını alır
  ...
  var table = $("#myTable").DataTable({
    ...
    ajax: {
      ...
      dataSrc: function (json) {
        ...
        if ($("#sessionsCount").length) {
          $("#sessionsCount").text(json.recordsFiltered);
        }
        return json.data;
      },
      data: function (d) {
        ...
        // Get status checkboxes that are checked
        const statuses = form
    ...
    columns: [
      ...
      {
        data: function (row) {
          // ✅ DB-dən gələn cihaz adını escape edirik
      ...
      {
        data: function (row) {
          // ✅ DB-dən gələn tarixi escape edirik
      ...
      {
        data: function (row) {
          // ✅ DB-dən gələn location məlumatını escape edirik
      ...
      {
        orderable: false, // ✅ Son sütun (üç nöqtə) filter off
        data: function (row) {
          // ✅ Session ID-ni də escape edirik (əlavə təhlükəsizlik üçün)
                                ...
                                <div onclick="toggleDropdown(this)" class="icon stratis-dot-vertical text-messages dark:text-primary-text-color-dark w-5 h-5 cursor-pointer"></div>
                                    ...
                                    <div class="relative h-[8px]">
                                      ...
                                      <div class="absolute top-1/2 right-4 w-3 h-3 bg-white dark:bg-menu-dark rotate-45 border-l border-t border-white dark:border-menu-dark z-50"
                                        style="box-shadow: -2px 0px 3px rgba(0,0,0,0.05), 2px 0px 3px rgba(0,0,0,0.05);">
                                    </div>
                                    </div>
  ...
  window.terminateSession = function (sessionId) {
    $.ajax({
      ...
      success: function (response) {
        table.ajax.reload();
        // Toast notification göstər (əgər varsa)
...
function openPopup(idS) {
  ...
  popup.innerHTML = `
    ...
    <div class="flex justify-end gap-2 w-full mt-2 text-xs font-medium">
      ...
      <button onclick="terminateSession('${idS}');$('.custom-popup').remove();" class="cursor-pointer px-3 py-1 rounded-full text-on-primary dark:text-on-primary-dark bg-error dark:bg-error-dark transition" id="confirmDeleteBtn">Bəli, bitir</button>
    </div>
  `;
