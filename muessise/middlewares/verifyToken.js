...
import { decrypt, generateFingerprint, extractDeviceInfo, compareFingerprints } from '../utils/crypto.js';
...
export const alreadyLoggedIn = async (req, res, next) => {
    ...
    try {
        ...
        if (mongoSession && mongoSession.otp_verified) {
            ...
            console.log("🟡 ALREADY LOGGED IN - User already authenticated, redirecting to:", redirectUrl);
            ...
            console.log("🟡 ALREADY LOGGED IN - OTP not verified, redirecting to otpVerify");
            return res.redirect('/auth/otpVerify');
        }
    } catch (err) {
        console.log("🔍 ALREADY LOGGED IN - Error occurred, ignoring:", err.message);
        // ignore and continue
    }
