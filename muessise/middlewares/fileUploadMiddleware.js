  ...
  return [
    ...
    async (req, res, next) => {
      try {
        ...
        const userId = req.user?.user_id || "P-TEST001"; // From user session
        const muessiseId = req.user?.muessise_id || "DEFAULT_MUESSISE"; // From user session
        ...
        for (const file of req.files) {
          try {
            ...
            const webRoute = `/files/uploads/${muessiseId}/${uniqueFilename}`;
            ...
            if (file.mimetype.startsWith("image/")) {
              ...
              const jpegRoute = `/files/uploads/${muessiseId}/${jpegFilename}`;
