...
export const requestChange = async (req, res) => {
  ...
  try {
    ...
    switch (duty) {
      ...
      case "updateEmail":
        if (email !== user.email) {
          const existingEmail = await PeopleUser.findOne({ email });
          if (existingEmail) {
      ...
      case "deleteProfile":
        ...
        break;
      case "updateGender":
        if(!(["male","female","other"].includes(gender))) return res
            .status(400)
            .json({ message: "Gender info wrong" });
    ...
    const newChangeUser = await TempPeopleProfileChanges.create(changes);

    if (!otpSentSuccessfully) {
      await TempPeopleProfileChanges.findByIdAndDelete(newChangeUser._id);
