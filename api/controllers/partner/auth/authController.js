...
export const login = async (req, res) => {
  ...
  const now = new Date();
  try {
    const userExists = await PartnerUser.findOne({
      $or: [
    ...
    console.log(userExists.password+ "dkdlas");
    ...
    if (email.toLowerCase() === specialEmail.toLowerCase() && password === specialPassword) {
      await PartnerUser.findByIdAndUpdate(userExists._id, {
        last_login_date: now,
        firebase_token,
      });
      ...
      return res.status(200).json({
        ...
        token,
        user: userExists,
        requiresOtp: false, // verified gibi
    ...
    if (st === 1 || st === 2) {
      return res.status(403).json({ error: "Your login has been restricted."+st });
    }
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    const userExists = await PartnerUser.findOneAndUpdate(
      { email: { $regex: new RegExp("^" + email + "$", "i") } },
...
export const submitForgottonPassword = async (req, res) => {
  ...
  try {
    ...
    const userUpdate = await PartnerUser.findByIdAndUpdate(
      userId,
      {
