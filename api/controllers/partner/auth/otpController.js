...
import { sendMail } from "../../../utils/optHandler.js";
...
export const submitOtp = async (req, res) => {
  ...
  try {
    ...
    const user = await PartnerUser.findOne({ email, _id: userId });
    if (!user) {
      return res
        .status(404)
    ...
    if (user.otp_destination === "email") {
      comingOtp = await OtpModel.findOne({ email: user.email }).sort({
        createdAt: -1,
      });
    ...
    if (session) {
      ...
      session.last_login_date = new Date();
      session.token_id = tokenId; // jti ekle
      await session.save();
    } else {
...
export const cancelOtp = async (req, res) => {
  try {
    ...
    // PartnerUser içindeki OTP bilgilerini sıfırla
    user.otp_code = null;
    user.otp_expires_at = null;
...
export const retryOtp = async (req, res) => {
  try {
    ...
    const comingOtp = await OtpModel.findOne({ email });
    if (!comingOtp)
      return res
        .status(404)
    ...
    await sendMail(email, user, otpCode, debug);
