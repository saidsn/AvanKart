import RbacPermission from "../models/rbacPermission.model.js";
...
const PERMISSION_MAPPING = {
  ...
  hesablashmalar_view: "accounting",
  company_info: "company_information",
  user_management: "users",
  roles: "role_groups",
  avankart_partner: "avankart_partner",
  profile: "profile",
...
const getUserPermissions = async (userId) => {
  ...
  try {
    ...
    if (!user) {
      isDev &&
        console.log(`[RBAC] PartnerUser not found for userId: ${userId}`);
      return null;
    }

    const muessiseOfUser = user.muessise_id;

    if (!muessiseOfUser) {
      isDev && console.log(`[RBAC] No muessise_id found for userId: ${userId}`);
      return null;
    }

    isDev && console.log(`[RBAC] User muessise_id: ${muessiseOfUser}`);

    const muessisePermission = await RbacPermission.findOne({
      muessise_id: muessiseOfUser,
    });

    if (!muessisePermission) {
      console.log(
        `[RBAC] No permissions found for muessise_id: ${muessiseOfUser}`
      );
      return null;
    }

    console.log(`[RBAC] Permissions found:`, muessisePermission.toObject());

    setCachedPermissions(userId, muessisePermission);
    return muessisePermission;
  } catch (err) {
...
export const checkRbacPermission = (
  ...
  return async (req, res, next) => {
    try {
      ...
      const user = await PartnerUser.findById(req.user.id);
      if (!user || !user.muessise_id) {
        console.log(`[RBAC MIDDLEWARE] User or muessise_id not found`);
        return res.redirect("/auth/login");
      }
      ...
      console.log(
        "[RBAC MIDDLEWARE] Muessise Permission:",
        muessisePermission?.toObject()
      );
      ...
      const permissionValue = muessisePermission[dbFieldName];
      ...
      if (!hasPermission) {
        ...
        const dashboardPermission = muessisePermission["dashboard"];
        console.log(
      ...
      req.userPermissions = muessisePermission;
      next();
    } catch (err) {
