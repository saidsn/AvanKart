...
export const notificationsPersonalTable = async (req, res) => {
  try {
    ...
    //  Əgər corporate tipidirsə, userId ilə PeopleUser tapıb sirket_id al
    if (category === "corporate") {
      const peopleUser = await PeopleUser.findById(userId); // PartnerUser → PeopleUser
      if (!peopleUser || !peopleUser.sirket_id) {
        // muessise_id → sirket_id
        return res.status(404).json({ error: "User or sirket not found" });
      }
      baseFilter.sirket_id = peopleUser.sirket_id; // muessise_id → sirket_id
    } else {
    ...
    const corporateTotal = await NotificationModel.countDocuments({
      sirket_id: (await PeopleUser.findById(userId))?.sirket_id, // muessise_id → sirket_id
      category: "corporate",
    });
...
export const notificationsPage = async (req, res) => {
  try {
    const userId = req.user?.id;
    const user = await PeopleUser.findById(userId); // PartnerUser → PeopleUser
...
export const setas = async (req, res) => {
  try {
    ...
    const user = await PeopleUser.findById(userId).select("sirket_id"); // PartnerUser → PeopleUser, muessise_id → sirket_id

    if (!user || !user.sirket_id) {
      // muessise_id → sirket_id
      return res.status(403).json({
        success: false,
        message: "Access Denied, User sirkete aid deyil.", // muessieye → sirkete
      });
    }
    ...
    console.error("setas error:", error);
    return res
      .status(500)
