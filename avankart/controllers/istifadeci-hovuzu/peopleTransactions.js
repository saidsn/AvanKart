...
export const getPeopleTransactionSubjects = async (req, res) => {
  try {
    ...
    const subjects = await TransactionsUser.distinct("subject", {
      from: peopleUser._id,
...
export const getPeopleTransactionCardCategories = async (req, res) => {
  try {
    const { people_id } = req.params;
...
export const getPeopleTransactionDestinationsAndStatuses = async (req, res) => {
  try {
    const { people_id } = req.params;
    ...
    const destinations = await TransactionsUser.distinct("destination", {
      from: peopleUser._id,
      destination: { $exists: true, $ne: null, $ne: "" },
    });

    const statuses = await TransactionsUser.distinct("status", {
      from: peopleUser._id,
      status: { $exists: true, $ne: null, $ne: "" },
    });

    const destinationMapOut = {
      Internal: "Mədaxil",
      External: "Məxaric",
    };
    const statusMapOut = {
    ...
    const mappedDestinations = destinations
      .map((dest) => ({
        value: destinationMapOut[dest] ? dest : dest,
        label: destinationMapOut[dest] || dest,
      }))
      .filter((dest) => dest.label);

    const mappedStatuses = statuses
      .map((status) => ({
        value: statusMapOut[status] ? status : status,
        label: statusMapOut[status] || status,
      }))
...
export const getPeopleTransactionsTable = async (req, res) => {
  try {
    const body = req.body || {};
    ...
    const dateFrom = startDate || date_from;
    const dateTo = endDate || date_to;
    if (dateFrom || dateTo) {
      match.date = {};
    ...
    if (
      ...
      const rawDest = destinationMapIn[destination] || destination;
      if (["Internal", "External"].includes(rawDest)) {
        match.destination = rawDest;
      }
    }
    ...
    if (statuses && Array.isArray(statuses) && statuses.length > 0) {
      ...
      const rawStatus = statusMapIn[status] || status;
      if (["success", "failed", "pending"].includes(rawStatus)) {
    ...
    if (
      ...
      requestedCatIds = toObjectIds(cardCategories);
    } else if (cardCategory) {
      requestedCatIds = toObjectIds(
        Array.isArray(cardCategory) ? cardCategory : [cardCategory]
      );
    }
