...
window.initMap = async function () {
  ...
  map.addListener("click", (event) => {
    ...
    document.getElementById(window.forAddress ?? 'latlng').value = `${latlng.lat.toFixed(6)}, ${latlng.lng.toFixed(6)}`;
    ...
    geocoder.geocode({ location: latlng }, (results, status) => {
      if (status === "OK" && results[0]) {
        document.getElementById(window.forHidden ?? 'address').value = results[0].formatted_address;
      } else {
        document.getElementById(window.forHidden ?? 'address').value = "Address not found";
      }
    });
    closeMapModal();
  });
