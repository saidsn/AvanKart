document.addEventListener("DOMContentLoaded", function () {
  const FAQ_ENDPOINT = "/muessise/faq/list";

  let faqData = [];

  const TEST_FAQ = [
    {
      question: "Test: Sistem nasil calisir?",
      answer: "Modal acilinca listeden soru-cevap olusturulur.",
    },
    {
      question: "Test: Dil parametresi nereden gelir?",
      answer: "Script src icindeki ?lang=... query parametresinden alinir.",
    },
    {
      question: "Test: Veri gelmezse ne olur?",
      answer: "Backend bos donerse bu test sorulari gosterilir.",
    },
  ];
  ...
  // Only run if FAQ modal element exists on the page
  ...
  postJSON(FAQ_ENDPOINT, { lang })
    .then((json) => {
      ...
      faqData = normalized.length ? normalized : TEST_FAQ;
    })
    .catch(() => {
      faqData = TEST_FAQ;
    });
  ...
  function openFaqModal() {
    ...
    if (listEl)
      listEl.innerHTML = renderFaqList(
        faqData && faqData.length ? faqData : TEST_FAQ
      );
  }
