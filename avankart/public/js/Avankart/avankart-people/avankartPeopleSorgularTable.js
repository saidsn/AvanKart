...
function renderStatusColumns(data) {
  ...
  statuses.forEach((status) => {
    ...
    data
      .filter((item) => {
        const mappedStatus = statusMap[item.status?.toLowerCase()] || item.status;
      ...
      .forEach((item) => {
        const initials = item.responsible
          ...
          const priority = priorityMap[item.priority?.toLowerCase()] || item.priority;
        const priorityIcon =
          priority === "High"
            ? '<img src="/images/Avankart/avankartPartner/high.svg" alt="High" class="w-4 h-4"/>'
            : priority === "Medium"
              ? '<img src="/images/Avankart/avankartPartner/medium.svg" alt="Medium" class="w-4 h-4"/>'
        ...
        const cardHtml = `
          <div onclick="window.location.href='../../destek/sorgu-detaylari.html'" class="cursor-pointer mb-2 p-3 bg-container-2 rounded-[8px]">
            ...
            <div class="mt-0.5 mb-3">
              ...
              <div class="text-[11px] font-normal opacity-65">${item.reason.name}</div>
            </div>
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2 justify-center">
                ${priorityIcon}
                <span class="text-[13px] font-medium">${priority}</span>
              </div>
            </div>
...
function loadQueriesData() {
  $.ajax({
    url: `/api/people/sorgular/${peopleId}`,
    type: "POST",
