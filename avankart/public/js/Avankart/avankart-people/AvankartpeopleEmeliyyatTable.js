...
$(document).ready(function () {
  ...
  function initializeDataTable() {
    if ($.fn.DataTable.isDataTable("#korporativTable")) {
      dataTable.destroy();
    }

    dataTable = $("#korporativTable").DataTable({
      ajax: {
      ...
      columns: [
        ...
        {
          data: null, // <-- no dataset property, because it's fixed
          render: function () {
            return '<span class="text-[13px] text-messages dark:text-on-primary-dark font-normal">5</span>';
          },
        },
        {
          data: "startDate",
  ...
  const $tableEmeliyyat = $("#tranzaksiyaEmeliyyatTable");
  const $tbodyEmeliyyat = $tableEmeliyyat.find("tbody");
  $tbodyEmeliyyat.html("");
  dataTranzaksiya.forEach((item) => {
    const colorStatusClass =
      item.status === "Uğurlu" ? "text-green-500" : "text-red-500";
    const colorAmountClass =
      item.appointment === "Mədaxil" && item.status === "Uğurlu"
        ? "text-massages"
        : "text-red-500";
    const sign =
      item.appointment === "Mədaxil" && item.status === "Uğurlu" ? "+" : "-";

    $tbodyEmeliyyat.append(`
            <tr class="bg-white border-b-1 dark:bg-menu-dark rounded-lg mb-2 mt-2" style="height:48px;">
                <td class="px-5 py-4 w-50 text-[13px] text-messages dark:text-primary-text-color-dark font-medium">
                    ${item.id}
                </td>
                <td class="px-5 py-4 w-40 text-[13px] text-messages dark:text-primary-text-color-dark font-normal">
                    ${item.appointment}
                </td>
                <td class="px-5 py-4 w-50 text-[13px] text-messages dark:text-primary-text-color-dark font-normal">
                    ${item.category}
                </td>
                <td class='px-5 py-4 w-150 text-[13px] dark:text-primary-text-color-dark font-normal ${colorAmountClass}'>
                    ${sign} ${item.amount}  
                </td>
                <td class="px-5 py-4 w-80 text-[13px] text-messages dark:text-primary-text-color-dark font-normal">
                    ${item.subject}
                </td>
                <td class="px-5 py-4 w-40 text-[13px] text-messages dark:text-primary-text-color-dark font-normal">
                    ${item.date}
                </td>
                <td class="px-5 py-4 w-40 text-[13px] ${colorStatusClass} dark:text-primary-text-color-dark font-normal">
                    ${item.status}
                </td>
            </tr>
        `);
  ...
  $tableEmeliyyat.find("thead th").css({
    "padding-top": "10.5px",
