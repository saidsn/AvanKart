...
$(document).ready(function () {
  ...
  function initializeContractsTable() {
    ...
    contractsTable = $("#contractsTable").DataTable({
      ajax: {
        url: `/sirketler/${window.companyId}/contracts`,
        type: "POST",
      ...
      columns: [
        {
          ...
          render: function (row) {
            console.log("row", row);
            return `
              <div class="relative rounded-[8px] py-5 px-3 flex flex-col items-center text-center w-full bg-sidebar-bg dark:bg-[#0C1418]">
                <!-- Trash icon -->
                <button 
                  class="absolute top-2 right-2 w-6 h-6 flex items-center justify-center hover:bg-error-hover rounded-full cursor-pointer"
                ...
                <div class="w-10 h-10 rounded-full bg-[#F0F0F0] flex items-center justify-center mb-3 dark:bg-[#161E22]">
                  <img src="${row.fileName}" alt="PDF" class="w-6 h-6">
                </div>

                <!-- Başlıq -->
                <p class="text-[13px] font-medium text-[#1D222BA6] mb-1 dark:text-[#FFFFFFA6]">Hesablaşma Müqaviləsi </p>

                <!-- Tarix və Saat -->
                <p class="text-[12px] font-normal text-[#1D222B80] mb-3 dark:text-[#FFFFFF80]">${row.createdAt}</p>

                <!-- Yüklə düyməsi -->
                <a href="/sirketler/contracts/download/${row._id}" download
      ...
      drawCallback: function () {
        ...
        if (pageInfo.pages > 1) {
          // Prev
          $pagination.append(
            `<div class="flex items-center justify-center px-3 h-8 ${pageInfo.page === 0
         ...
         <div class="icon stratis-chevron-left text-xs"></div>
          ...
          for (let i = 0; i < pageInfo.pages; i++) {
            paginationButtons +=
              `<button class="cursor-pointer w-10 h-10 rounded-[8px] ${i === pageInfo.page
          ...
          // Next
          $pagination.append(
            `<div class="flex items-center justify-center px-3 h-8 ${pageInfo.page === pageInfo.pages - 1
              ? "text-[#BFC8CC] dark:text-[#636B6F] cursor-not-allowed"
              : "text-messages dark:text-[#FFFFFF] cursor-pointer"
...
window.applyContractsFilters = async function () {
  ...
  const contracts = await fetch(`/sirketler/${window.companyId}/contracts`, {
    method: "POST",
    headers: {
  ...
  const data = await contracts.json();
  ...
  if (contracts) {
    ...
    if (contractsTable) {
      ...
      contractsTable.rows.add(data.data); // rows must match your columns/data expectations
      contractsTable.draw();
    }
