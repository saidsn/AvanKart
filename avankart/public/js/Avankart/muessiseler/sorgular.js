...
function renderStatusColumns(data,muessiseId) {
  ...
  statuses.forEach((status) => {
    ...
    data
      ...
      .forEach((item) => {
        ...
        const cardHtml = `
          <div onclick="window.location.href='sorgular/${item.ticket_id}'" class="cursor-pointer mb-2 p-3 bg-container-2 rounded-[8px]">
            <div class="flex items-center justify-between">
              <span class="text-[11px] opacity-65">${item.ticket_id}</span>
            </div>
...
function loadQueriesData(m_id) {
  const csrfToken =
    ...
    success: function (response) {
      ...
      renderStatusColumns(queriesData,muessiseId);
    },
    error: function () {
