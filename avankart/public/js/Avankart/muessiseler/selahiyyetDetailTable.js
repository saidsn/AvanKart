...
$(document).ready(function () {
  ...
  function initializeselahiyyetDetailDataTable() {
    ...
    selahiyyetDetailTable = $("#selahiyyetDetailTable").DataTable({
      ajax: {
        url: "/muessiseler/selahiyyet-detail-table",
        type: "POST",
      ...
      columns: [
        ...
        },
        {
          data: "position",
          render: function (data, type, row) {
            return `<span class="text-[13px] text-messages dark:text-primary-text-color-dark font-normal">${row.duty}</span>`;
          },
        },
        {
          data: "email",
      ...
      createdRow: function (row, data, dataIndex) {
        $(row)
        $("$backToIStifadecilerContent").text()
  ...
  //   window.selahiyyetDetailTable.draw()
  // }
  ...
  $("#selahiyyetDetailrefreshBtn").on("click", function (){
    if(selahiyyetDetailTable) {
      selahiyyetDetailTable.ajax.reload(null,false)
    }
  })
