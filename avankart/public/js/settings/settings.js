...
function toggleEmail(formName) {
  console.log(formId, "form id toggle emailden");
  console.log(toggleclick, "clicklable -ye baxiriq toogleEmail funcdan");
  toggleclick = !toggleclick;
  if (toggleclick) {
    submitForm(formName);
...
document.addEventListener("DOMContentLoaded", function () {
  // Mövcud submitForm funksiyasını wrap edirik
  if (
    typeof window.originalSubmitForm === "undefined" &&
    typeof submitForm === "function"
  ) {
    window.originalSubmitForm = submitForm;

    // submitForm-u yenidən təyin edirik callback ilə
    window.submitForm = function (formId) {
      ...
      if (form) {
        ...
        if (url) {
          fetch(url, {
            ...
            .then((responseData) => {
              if (responseData.qr)
                $("#qrImageCont").attr("src", responseData.qr);
              if (responseData.qr) $("#qrImageCont").removeClass("hidden");
              if (!responseData.qr) $("#qrImageCont").addClass("hidden");
              // YENİ: OTP callback çağır
              if (typeof handleFormResponse === "function") {
                handleFormResponse(formId, null, responseData);
              }

              // Mövcud sistemə uyğun response handling
      ...
      // Orijinal funksiyaya da müraciət et (fallback)
      if (
        window.originalSubmitForm &&
        typeof window.originalSubmitForm === "function"
      ) {
