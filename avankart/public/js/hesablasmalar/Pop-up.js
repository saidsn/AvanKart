...
const transparentElements = [
  aside, span4, span4_1, curveleft, tenzimlemelerDiv, destekDiv, destekLogo
];
...
function confirmAvankarta() {
  ...
  .then(data => {
    if (data.success) {
      alert(data.message || 'Invoice uğurla təsdiqləndi');
...
function confirmDeleteInvoice(otp) {
  ...
  .then(data => {
    if (data.success) {
      alert(data.message || 'Invoice uğurla silindi');
      location.reload(); // Səhifəni yenilə
    } else {
      alert(data.message || 'Xəta baş verdi');
    }
