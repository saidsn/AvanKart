...
function handleFileUpload(file) {
  ...
  setTimeout(() => {
    ...
    loadingText.innerHTML =
      "<div id='dark'>Yüklənir... 100% | 0 saniyə qalıb</div>";
...
$(function () {
  if ($("#slider-range-2").length) {
    ...
    $("#min-value-2").text(
      formatCurrency($("#slider-range-2").slider("values", 0))
    );
    $("#max-value-2").text(
      formatCurrency($("#slider-range-2").slider("values", 1))
    );
  }
});
...
$(function () {
  if ($("#slider-range-3").length) {
    ...
    $("#min-value-3").text(
      formatCurrency($("#slider-range-3").slider("values", 0))
    );
    $("#max-value-3").text(
      formatCurrency($("#slider-range-3").slider("values", 1))
    );
  }
});
    ...
    allForms.forEach((form) => {
      ...
      if (form.querySelector("#slider-range-3")) {
        ...
        newForm.addEventListener("reset", function (e) {
          ...
          const formCheckboxes = newForm.querySelectorAll(
            'input[type="checkbox"]'
          );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (dropdownButton78 && dropdownMenu78) {
    ...
    document.addEventListener("click", (event) => {
      if (
        !dropdownButton78.contains(event.target) &&
        !dropdownMenu78.contains(event.target)
      ) {
...
function openDeleteInvoiceModal() {
  document
    .getElementById("deleteInvoiceModalOverlay")
