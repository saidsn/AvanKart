document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTableVisibility(id, show = false, permName) {
        if (!id) return;
                    ...
                    data: function(row, type, set, meta) {
                        ...
                        return `
                            <div class="flex justify-center">
                                <input type="checkbox" class="hidden" data-hidden-toggle="true" data-row-id="${row._id}" data-user-id="${row.id}" data-table-id="#perm_user_table_${row.duty}" id="is_deleted_${row._id}" ${row.isHidden ? 'checked' : ''}>
    ...
    window.saveThisTable = () => {
        const main_values = {};      // perm_id -> { card_id: value }
    ...
    function updateBalanceDisplay(permId, cardId) {
    ...
    function updateTotalBalance(permId) {
        let totalBalance = 0;
        sirketCards.forEach(card => {
    ...
    function updateAllTotal() {
        // Kart bazlı toplamları hesapla
        let kartBazliToplamlar = {}; // {cardId: toplamBalance}

        Object.values(kartlarBalance).forEach(cardsObj => {
        ...
        Object.entries(kartBazliToplamlar).forEach(([cardId, toplam]) => {
        ...
        console.log("Kartlar üzerinden genel toplam:", kartlarGenelToplam.toFixed(2));
    ...
    // DÜZENLENMİŞ calculateBalance FONKSİYONU
    calculateBalance = (permId, cardId) => {
        ...
        if (data.exceptions[permId]) {
            ...
            console.log(`Hidden users for permId ${permId}:`, hiddenUsers);
        ...
        console.log(`Final total balance for ${permId}.${cardId}: ${finalBalance} (Active users: ${activeUserCount})`);
        return finalBalance;
    };
