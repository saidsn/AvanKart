$(document).ready(function () {
    ...
    table = $("#myTable").DataTable({
      ...
      ajax: {
        ...
        dataSrc: function (json) {
          const rows = Array.isArray(json?.data) ? json.data : json?.rows || [];
          try {
      ...
      columns: [
        // İnvoys nömrəsi
        {
          ...
          render: function (_, __, row) {
            const inv =
              row.balance_id || row.invoiceNo || row.invoice || row.id || "-";
        ...
        // Kart sayı
        {
          ...
          render: function (_, __, row) {
            const cnt =
              row.cards ??
              row.cardCount ??
              (Array.isArray(row.cards) ? row.cards.length : null);
  ...
  function updateStatistics(stats) {
    ...
    const totalRecords = table ? table.page.info().recordsTotal : stats.total;
  ...
  window.confirmInvoice = function (balanceId) {
    alert(`Invoysu təsdiqlə klikləndi: #${balanceId}`);
  };

  window.deleteInvoice = function (balanceId) {
    alert(`Sil klikləndi: #${balanceId}`);
  };
