$(document).ready(function () {
    ...
    const table = $('#myTable').DataTable({
        ...
        },
        {
            data: function(row) {
                return `
                <div class="flex items-center gap-1 cursor-pointer text-messages">
        ...
        {
            data: function(row) {
                const isSuccess = row.status === 'Uğurlu';
                return `
                    <div class="flex items-center gap-2">
                        <span class="w-[6px] h-[6px] rounded-full ${isSuccess ? 'bg-success' : 'bg-error'}"></span>
                        <div class="flex items-center gap-7.5 justify-between">
                            <span class="text-[13px] font-medium ${isSuccess ? 'text-success' : 'text-error'}">${row.status}</span>
                        ${!isSuccess ? '<div class="iconex iconex-info-circle-1 text-error w-5 h-5"></div>' : ''}
                        </div>
                    </div>
                `;
            }
