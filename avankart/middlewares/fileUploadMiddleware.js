...
// Main middleware function kohne versiya
...
//           } catch (error) {
//             console.log(
//               "PeopleUser tapılmadı, default sirket_id istifadə olunur"
//             );
//           }
//         }
...
//         console.log(`🏢 Sirket ID: ${sirketId}`); // Muessise ID → Sirket ID

//         // Create upload directory - sirket_id istifadə olunur
...
//             const webRoute = `/files/uploads/${sirketId}/${uniqueFilename}`; // muessiseId → sirketId
...
//               const jpegRoute = `/files/uploads/${sirketId}/${jpegFilename}`; // muessiseId → sirketId
...
//         // Add processed files to request - sirketId istifadə olunur
...
//   ];
// };
// Yeni versiya (yeni versiya ne ise yarayir -> edit zamani frontenden yeni sekil gonderilmedikde xeta verirdi bu icaze verirki edit zamani sorgu atilanda xeta qaytarmasin)
...
const fileUploadMiddleware = (fieldName = "files") => {
  return [
    ...
    async (req, res, next) => {
      try {
        const isEditRequest = req.method === 'PUT' || req.method === 'PATCH';

        if (!req.files || req.files.length === 0) {
          console.log("----req.files-----", req.files);
          
          if (isEditRequest) {
            console.log("ℹ️ Edit request - fayl yüklənməyib, mövcud şəkil saxlanılacaq");
            req.processedFiles = [];
            return next();
          }
        ...
        const myUser = AdminUser.findById(req.user.id);
        const userId = myUser.user_id ?? "P-TEST001"; 
        ...
        for (const file of req.files) {
          try {
            ...
            const filePath = path.join(uploadDir, uniqueFilename);
            const webRoute = `/files/uploads/${sirketId}/${uniqueFilename}`;
            ...
            if (file.mimetype.startsWith("image/")) {
              ...
              const jpegPath = path.join(uploadDir, jpegFilename);
              const jpegRoute = `/files/uploads/${sirketId}/${jpegFilename}`;
