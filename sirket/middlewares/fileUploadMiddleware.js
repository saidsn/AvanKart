...
const generateUniqueFilename = (userId, originalName) => {
  ...
  const userIdFormat = userId ? userId.toString() : "P-TEST001";
    ...
    async (req, res, next) => {
      ...
      try {
        if (!req.files || req.files.length === 0) {
          console.log("No files uploaded, proceeding without files");
          req.processedFiles = [];
          req.uploadMeta = {
          ...
          console.log("🔧 File upload middleware completed (no files)");
          return next();
        }

        const { ticketId } = req.params;

        // Get user information properly
        const userId = req.user?.id;
        if (!userId) {
          throw new Error("User ID not found in request");
        }

        // Get user and sirket_id
        const myUser = await PeopleUser.findById(userId);
        if (!myUser) {
        ...
        if (!sirketId) {
          throw new Error("User does not have a sirket_id");
        }
        ...
        const uploadDir = path.join(
          ...
          "files",
          "uploads",
          sirketId.toString() // Convert ObjectId to string for path
        );
        ...
        for (const file of req.files) {
          try {
            ...
            const webRoute = `/files/uploads/${sirketId.toString()}/${uniqueFilename}`; // Convert ObjectId to string
            ...
            if (file.mimetype.startsWith("image/")) {
              ...
              const jpegRoute = `/files/uploads/${sirketId.toString()}/${jpegFilename}`; // Convert ObjectId to string
