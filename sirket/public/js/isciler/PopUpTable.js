$(document).ready(function () {
  ...
  const table = $("#myTablePop").DataTable({
    ...
    dom: "t",
    ajax: {
      url: "/people/avankart-people-imtiyaz",
      type: "POST",
    ...
    columns: [
      ...
      {
        data: function (row) {
          return `<span class="text-[13px] text-messages font-normal dark:text-white">${row.fullname}</span>`;
        },
      },
      {
        data: function (row) {
      ...
      {
        data: function (row) {
          return `<span class="text-[13px] text-messages font-normal dark:text-white">${row.permission}</span>`;
        },
      },
      {
        data: function (row) {
