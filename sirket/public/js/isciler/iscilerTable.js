$(document).ready(function () {
  ...
  function updateButtonsContainerForSelection(tableType) {
    ...
    if (tableType === "emekdaslar") {
      ...
      if (currentSubTab === "aktiv") {
        ...
        additionalButtonsContainer.html(`
          <div class="h-[34px] hidden border border-surface-variant rounded-full flex items-center hover:text-primary gap-2 px-2 text-messages dark:bg-table-hover-dark dark:border-on-surface-variant dark:text-white cursor-pointer" onclick="exportSelectedToExcel()">
            <img src="/images/uzv-sirket/isciler/excelIcon.svg" alt="Excel icon">
            <span class="text-xs font-medium">Seçilənləri Excelə çıxart</span>
          </div>
        `);
      ...
      buttonsContainer.empty();
      additionalButtonsContainer.html(`
        <div class="h-[34px] border hidden border-surface-variant rounded-full flex items-center hover:text-primary gap-2 px-2 text-messages dark:bg-table-hover-dark dark:border-on-surface-variant dark:text-white cursor-pointer" onclick="exportSelectedToExcel()">
          <img src="/images/uzv-sirket/isciler/excelIcon.svg" alt="Excel icon">
          <span class="text-xs font-medium">Seçilənləri Excelə çıxart</span>
        </div>
      `);
  ...
  window.showDeleteItemPopup = function(){
    alertModal('Bunu etmək mümkün deyil', 'error')
  }
    ...
    emekdaslarTable = $("#myTable").DataTable({
      ...
      ajax: {
        ...
        dataSrc: function (json) {
          if (currentSubTab === "aktiv") {
            $("#aktivButton").html(
              "Aktiv işçilər (" + (json.recordsTotal ?? 0) + ")"
            );
          } else if (currentSubTab === "ayrilan") {
            $("#ayrilanlarButton").html(
              "İşdən ayrılanlar (" + (json.recordsTotal ?? 0) + ")"
            );
          }
          return json.data || [];
        },
      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data, type, row) {
            ...
            <span class="w-[6px] h-[6px] rounded-full ${row.isInvite ? row.inviteRawStatus === 'rejected' ? "bg-[#EF5350]": "bg-[#FFCA28]" : "hidden"} shrink-0 mr-2"></span> 
            ${
              row.isInvite ? row.inviteStatus : row.hireDate ? row.hireDate : "-"
            }</span></div>`;
          },
        },
        {
      ...
      createdRow: function (row, data, dataIndex) {
        ...
        $(row).on("click", "td:not(:first-child):not(:last-child)", function (e) {
  ...
  window.viewImtiyaz = function (balanceId) {
    if (balanceId) {
      window.location.href = `/isci/imtiyaz/${balanceId}`;
    } else {
      console.error("Balance ID tapılmadı");
    }
  };
  window.viewDuty = function (balanceId) {
    if (balanceId) {
      window.location.href = `/isci/duty/${balanceId}`;
    } else {
      console.error("Balance ID tapılmadı");
    }
  };
  // Global function to refresh imtiyaz table
  window.refreshImtiyazTable = function () {
    console.log("Manually refreshing imtiyaz table");
    if (imtiyazTable) {
      try {
        if (
          imtiyazTable.ajax &&
          typeof imtiyazTable.ajax.reload === "function"
        ) {
          console.log("Reloading imtiyaz table via AJAX");
          imtiyazTable.ajax.reload(null, false); // false = don't reset paging
        } else {
          console.log("No AJAX available, reinitializing imtiyaz table");
          initializeImtiyazTable();
        }
      } catch (err) {
        console.error("Error refreshing imtiyaz table:", err);
        initializeImtiyazTable();
      }
    } else {
      console.log("imtiyazTable not found, initializing new table");
      initializeImtiyazTable();
    }
  };

  // Helper function to refresh imtiyaz table with tab switching
  window.reloadImtiyazTableWithTabSwitch = function () {
    console.log("Reloading imtiyaz table with tab switch check");

    // Check current tab using DOM elements
    const imtiyazTab = document.getElementById("imtiyazTab");
    const isImtiyazTabActive =
      imtiyazTab && imtiyazTab.classList.contains("bg-inverse-on-surface");

    if (!isImtiyazTabActive) {
      console.log("Switching to imtiyaz tab first");
      if (typeof window.switchTab === "function") {
        window.switchTab("imtiyaz");
      }
      setTimeout(() => {
  ...
  function initializeImtiyazTable() {
    ...
    // Make imtiyazTable globally accessible
    window.imtiyazTable = imtiyazTable;
  }
  ...
  function createDropdown(rowIndex, tableType, name, id) {
    return `
     <div class="flex">
            ...
            <div class="dropdown-menu absolute right-[-12px] w-[122px] z-50 hidden" id="dropdown-${tableType}-${rowIndex}">
              ...
              <div style="box-shadow: 0px 0px 0px rgba(0,0,0,0.1), 0px 2px 2px rgba(0,0,0,0.1);" class="w-[122px] rounded-[8px] shadow-lg bg-white dark:bg-menu-dark overflow-hidden relative z-50">
                  <div class="py-[3.5px] text-[13px]">
                      <div class="text-messages dark:text-primary-text-color-dark flex flex-col gap-1 cursor-pointer" id="dropdown-content-${tableType}-${rowIndex}" data-name="${name}" data-id="${id}">
  ...
  function updateDropdownContent(rowIndex, tableType, rowData) {
    ...
    if (tableType === "emekdaslar") {
      ...
      const peopId = rowData?.people_id || rowData?.id || "";

      if (currentSubTab === "aktiv") {
        dropdownContent.html(`
          <span onclick="openEditPopup('${objectId || ""}')" class="hidden flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-[#F5F5F5] dark:hover:bg-[#353945] text-[#1D222B] dark:text-white text-[13px] font-normal">
            <div class="icon stratis-edit-02 text-messages dark:text-primary-text-color-dark w-4 h-4"></div>
            <span>Redaktə et</span>
          </span>
          <div class="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-[#F5F5F5] dark:hover:bg-[#353945] text-[#1D222B] dark:text-white text-[13px] font-normal" onclick="openBalancePopup('${peopId || ""}')">
          ...
          <div class="flex items-center gap-2 px-3 py-2 cursor-pointer text-[#DD3838] hover:bg-error-hover dark:hover:bg-error-hover-dark text-[13px] font-normal" onclick="openDeleteInvoiceModal('${objectId || ""}')">
            ...
            <span class="text-error dark:text-error-dark">İşçini sil</span>
          </div>
        `);
      } else {
        dropdownContent.html(`
    ...
    <span class="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-[#F5F5F5] dark:hover:bg-[#353945] text-[#1D222B] dark:text-white text-[13px] font-normal"
          onclick="console.log('Rename clicked for gid:', '${gid}'); openSimpleRenameModal('${gid}')">
      <div class="icon stratis-edit-03 text-[#1D222BA6] dark:text-primary-text-color-dark w-4 h-4"></div>
      <span>Adı redaktə et</span>
    </span>
    <div class="border-t dark:border-surface-variant-dark"></div>
    <div class="flex items-center gap-2 px-3 py-2 cursor-pointer text-[#DD3838] hover:bg-error-hover dark:hover:bg-error-hover-dark text-[13px] font-normal"
         onclick="console.log('Delete clicked for gid:', '${gid}'); openDeleteGroupModal('${gid}')">
      <div class="icon stratis-trash-01 text-error dark:text-error-dark w-4 h-4"></div>
      <span class="text-error dark:text-error-dark">Sil</span>
    </div>
  `);
    } else if (tableType === "vezife") {
      let name = dropdownContent.attr("data-name");
      let id = dropdownContent.attr("data-id");
      dropdownContent.html(`
    <div onclick="openVezifeEdit('${id}','${name}')" class="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-[#F5F5F5] dark:hover:bg-[#353945] text-[#1D222B] dark:text-white text-[13px] font-normal">
      <div class="icon stratis-file-edit-02 text-[#1D222BA6] dark:text-primary-text-color-dark w-4 h-4"></div>
      <span>Redaktə et</span>
    </div>
    <div class="border-t dark:border-surface-variant-dark"></div>
    <div onclick="openVezifeDelete('${id}')" class="flex items-center gap-2 px-3 py-2 cursor-pointer text-[#DD3838] hover:bg-error-hover dark:hover:bg-error-hover-dark text-[13px] font-normal">
      <div class="icon stratis-trash-01 text-error dark:text-error-dark w-4 h-4"></div>
      <span class="text-error dark:text-error-dark">Sil</span>
    </div>
  `);
  ...
  $("#refreshPage").on("click", function() {
    emekdaslarTable.ajax.reload();
  });
  $("#refreshDutyPage").on("click", function() {
    vezifeTable.ajax.reload();
  });
  $("#refreshImtiyazPage").on("click", function() {
    imtiyazTable.ajax.reload();
  });
  ...
  function updateButtonsContainer() {
    ...
    if (currentMainTab === "emekdaslar") {
      buttonsContainer.html(`
        <div class="flex !mb-[12.5px] mt-[12.5px] gap-2 items-center border border-surface-variant rounded-full p-1 dark:bg-table-hover-dark dark:border-on-surface-variant">
          <button id="aktivButton" class="px-1 py-[3px] text-messages dark:text-secondary-text-color-dark text-[12px] rounded-full font-medium hover:dark:text-primary-text-color-dark cursor-pointer ${
            currentSubTab === "aktiv"
          ...
          <button id="ayrilanlarButton" class="px-1 py-[3px] text-messages dark:text-secondary-text-color-dark text-[12px] rounded-full font-medium hover:dark:text-primary-text-color-dark cursor-pointer ${
        ...
        <div id="refreshPage"
          class="w-[155px] border cursor-pointer border-surface-variant dark:border-surface-variant-dark rounded-full flex items-center justify-center gap-2 text-[13px] text-messages dark:text-primary-text-color-dark hover:text-focus dark:hover:text-focus-color-dark font-medium py-[6.5px] px-[18px]"
        >
   ...
   <div id="balansArtirButton" class="h-[34px] border border-surface-variant rounded-full flex items-center hover:text-primary gap-2 px-2 text-messages dark:bg-table-hover-dark dark:border-on-surface-variant dark:text-white ${
     currentSubTab === "ayrilan" ? "pointer-events-none opacity-50" : ""
   }">
    <a href="/isci/add-balance" class="icon stratis-card-add !w-[12px] !h-[9px] text-messages hover:text-messages dark:text-white ${
      currentSubTab === "ayrilan" ? "text-[#BFC8CC]" : ""
    }"></a>
    <a href="/isci/add-balance" class="text-[13px] font-medium ${
      currentSubTab === "ayrilan" ? "text-[#BFC8CC]" : ""
    }">Balans artır</a>
</div>
        <div class="hidden h-[34px] border border-surface-variant rounded-full flex items-center hover:text-primary gap-2 px-2 text-messages dark:bg-table-hover-dark dark:border-on-surface-variant dark:text-white" onclick="excelPopUp()">
          <img src="/images/uzv-sirket/isciler/excelIcon.svg" alt="Excel icon">
  ...
  window.openVezifeEdit = function (id, name) {
    const el = document.getElementById("RedakteEtPopUpVezife");
    if (el) el.classList.remove("hidden");
    document
      .querySelectorAll(".dropdown-menu")
      .forEach((m) => m.classList.add("hidden"));
    $("#vezife-adi-popup-name").val(name);
    $("#vezife-adi-popup-id").val(id);
  };

  window.openVezifeDelete = function (id) {
    const el = document.getElementById("VezifeSilPop");
    if (el) el.classList.remove("hidden");
    document
      .querySelectorAll(".dropdown-menu")
      .forEach((m) => m.classList.add("hidden"));
    if (id) document.getElementById("vezife-sil-popup-id").value = id;
  };
  ...
  function initializeDropdown1() {
    ...
    const dropdownButton1 = popup
      ? popup.querySelector("#dropdownDefaultButton")
      : document.getElementById("dropdownDefaultButton");
    const dropdown1 = popup
      ? popup.querySelector("#dropdown1")
  ...
  function initializeDropdown2() {
    // Look for elements within the popup
    const popup = document.getElementById("EmekdaslarRedaktePop");
    const dropdownButton2 = popup
      ? popup.querySelector("#dropdownDefaultButton2")
      : document.getElementById("dropdownDefaultButton2");
    const dropdown2 = popup
      ? popup.querySelector("#dropdown2")
  ...
  function clearEditPopupData() {
    ...
    fieldRows.forEach((row) => {
      ...
      const inputOrSelect = row.querySelector(
  ...
  window.openEditPopup = function (objectId) {
    if (!objectId) {
      alertModal("Xəta: İstifadəçi ID-si tapılmadı", 'error');
      return;
    }
    ...
    try {
      table.rows().every(function (index) {
        ...
        if (data) {
          ...
          const matchingId = possibleIds.find(
            (id) =>
              id === objectId ||
      ...
      alertModal("Xəta: Cədvəl məlumatlarına çıxış alınmadı", 'error');
      return;
    }
    ...
    if (popup) {
      ...
      alertModal("Xəta: Redaktə pəncərəsi tapılmadı", 'error');
    }
  };
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          alertModal("Xəta: İstifadəçi məlumatları alınmadı - " + response.message, 'error');
        }
      },
      error: function (xhr, status, error) {
        alertModal("Xəta: Server ilə əlaqə problemi", 'error');
      },
    });
  }
  ...
  function populateEditPopup(rowData) {
    ...
    window.currentEditingObjectId =
      rowData._id || rowData.objectId || rowData.id;
    ...
    fieldRows.forEach((row, index) => {
      ...
      if (!valueElement) {
        valueElement = row.querySelector(
          'input[id^="employee-"], select[id^="employee-"]'
        );
        if (valueElement) {
      ...
      if (labelText.includes("Adı") || labelText.includes("Ad")) {
        // Name field
        const name = getFieldValue(rowData, [
          "name",
          "fullname",
          "firstName",
          "ad",
        ]);
        const surname = getFieldValue(rowData, [
          "surname",
          "lastName",
          "soyad",
        ]);
        ...
        inputElement = createEditableInput(
          "email",
          fieldValue,
          "employee-email"
        );
        ...
        // Phone field
        fieldValue = getFieldValue(rowData, [
          "phone",
          "phoneNumber",
          "telefon",
          "phone_suffix",
        ...
        labelText.includes("Vəzifəsi") ||
        labelText.includes("Vəzifə qrupu")
      ) {
        // Skip dropdown fields - they will be handled separately
        return;
      } else {
        // Keep other fields as spans for now
        valueSpan.textContent = getFieldValue(rowData, [
    ...
    const position = getFieldValue(rowData, [
      "position",
      "vezife",
      "job",
      "title",
    ...
    const department = getFieldValue(rowData, [
      "department",
      "vezife_qrupu",
      "group",
      "departman",
    ...
    if (department) {
      setTimeout(() => {
        const departmentButton = document.getElementById(
  ...
  function createEditableInput(type, value, id) {
    ...
    input.id = id;
    input.className =
      "font-medium dark:text-white bg-transparent border-b border-gray-300 dark:border-gray-600 focus:border-primary focus:outline-none px-1 py-1";
  ...
  function collectEditedData() {
    ...
    if (phoneInput) {
      ...
      if (cleanPhone.length >= 7 && cleanPhone.length <= 15) {
        ...
        alertModal("Telefon nömrəsi 7-15 rəqəm arasında olmalıdır", 'error');
  ...
  window.saveEditedEmployee = function () {
    ...
    if (!editedData || !editedData.objectId) {
      alertModal("Xəta: Məlumatları toplamaq mümkün olmadı", 'error');
      return;
    }
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          alertModal("İşçi məlumatları uğurla yeniləndi", 'error');
          ...
          if (response.otpRequired) {
            Otp(response.email, response.tempRecordId, {
              url: "muessise-info/accept-edit",
            });
            alertModal("otp acildi");
            return;
          }
          ...
          alertModal(
            "Xəta baş verdi: " +
              (response.message || "Məlumatları yeniləmək mümkün olmadı", 'error')
          );
        }
      },
      error: function (xhr, status, error) {
        alertModal("Məlumatları yeniləmə zamanı xəta baş verdi", 'error');
      },
    });
  };
  ...
  window.openBalancePopup = function (objectId) {
    let qamma = false;
    if (!objectId) {
      // seçilmişləri objectidyə eşitle
      objectId = Array.from(document.querySelectorAll('input[type="checkbox"][id^="cb-active-"]'))
      .filter(cb => cb.checked) // sadece işaretlenenleri almak istersen
      .map(cb => cb.dataset.peopId);
      qamma = true
    }

    if (!objectId || (Array.isArray(objectId) && objectId.length === 0)) {
        alertModal("Heç bir istifadəçi seçilməyib", "error");
        return;
    }

    let ajUrl;
    currentObjectId = objectId;
    Array.isArray(objectId) ? ajUrl = `/people/user-data/objectU` : ajUrl = `/people/user-data/${objectId}`
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          qamma ? populateBalanceAllPopup(response) : populateBalancePopup(response);

          $("#balancePopup").removeClass("hidden").addClass("flex");
        } else {
          alertModal(
            "İstifadəçi məlumatları alınarkən xəta baş verdi: " +
              (response.message || "Məlumat tapılmadı", 'error')
          );
        }
      },
      error: function (xhr, status, error) {
        alertModal("İstifadəçi məlumatları alınarkən xəta baş verdi", 'error');
      },
    });
  };
  ...
  function populateBalancePopup(userData) {
    ...
    const cardsElement = $('#cardHolderDivGo');
    if (descriptionElement && userData.people_id) {
      let u = userData;
      let userBox = '';
      let cardsBox = '';
      // Model: PeopleUser (name, surname, email, phone_suffix, phone, gender, people_id ...)
    const name       = u.name ?? splitName(u.fullname).name;
    const surname    = u.surname ?? splitName(u.fullname).surname;
    const genderText = formatGender(u.gender);
    const email      = u.email ?? "-";
    const phoneText  = (u.phone_suffix && u.phone) ? `+${u.phone_suffix} ${u.phone}` : (u.phoneNumber ?? u.phone ?? "-");
    const userId     = u._id ?? u.id ?? "";

    userBox = `
      <div class="flex justify-between border-b dark:border-[#FFFFFF1A] pb-3">
        <span class="opacity-65">Adı:</span><span class="font-medium">${name}</span>
      </div>
      <div class="flex justify-between border-b dark:border-[#FFFFFF1A] py-3">
        <span class="opacity-65">Soyad:</span><span class="font-medium">${surname}</span>
      </div>
      <div class="flex justify-between border-b dark:border-[#FFFFFF1A] py-3">
        <span class="opacity-65">Cinsi:</span><span class="font-medium">${genderText}</span>
      </div>
      <div class="flex justify-between border-b dark:border-[#FFFFFF1A] py-3">
        <span class="opacity-65">Email:</span><span class="font-medium">${email}</span>
      </div>
      <div class="flex justify-between border-b dark:border-[#FFFFFF1A] py-3">
        <span class="opacity-65">Telefon:</span><span class="font-medium">${phoneText}</span>
      </div>
      <input type="hidden" id="hiddenUserId" name="user_id[]" value="${userId}" />
    `;
      // <div class="flex items-center justify-between w-full">
      //     <span class="dark:text-white opacity-50 text-messages text-[12px]">
      //         Yemək kartı:
      //     </span>
      //     <input type="number" autocomplete="off" class="cursor-pointer dark:text-primary-text-color-dark max-w-[120px] h-[34px] font-poppins font-normal text-[12px] leading-[160%] border-[1px] border-surface-variant rounded-full pl-[12px] placeholder-on-surface-variant-dark hover:bg-input-hover focus:border-focus focus:shadow-[0px_0px_0px_2.5px_var(--color-button-disabled)] focus:ring-0 focus:outline-none active:border-focus active:shadow-[0px_0px_0px_2.5px_var(--color-button-disabled)] transition-all ease-out duration-300 dark:bg-table-hover-dark dark:border-on-surface-variant" placeholder="Məbləğ daxil edin" name="cards[6874defeb51862145dbb1e3a]">
      // </div>
    if(u.cards && u.cards.length > 0){
      u.cards.forEach(car => {
        ...
        <div class="flex items-center justify-between w-full">
            ...
            <input type="number" name="cards[${car._id}]" autocomplete="off" class="cursor-pointer dark:text-primary-text-color-dark max-w-[120px] h-[34px] font-poppins font-normal text-[12px] leading-[160%] border-[1px] border-surface-variant rounded-full pl-[12px] placeholder-on-surface-variant-dark hover:bg-input-hover focus:border-focus focus:shadow-[0px_0px_0px_2.5px_var(--color-button-disabled)] focus:ring-0 focus:outline-none active:border-focus active:shadow-[0px_0px_0px_2.5px_var(--color-button-disabled)] transition-all ease-out duration-300 dark:bg-table-hover-dark dark:border-on-surface-variant" placeholder="Məbləğ daxil edin" name="cards[6874defeb51862145dbb1e3a]">
        </div>
        `
      });
    }

      descriptionElement.innerHTML = userBox;
      cardsElement.html(cardsBox);
  ...
  function populateBalanceAllPopup(userData) {
    console.log("userData:", userData);
    ...
    // Update the popup title to include user name and ID
    const titleElement = popup.querySelector("h2");
    const descriptionElement2 = popup.querySelector("p");
    ...
    // Update the description to include user ID for reference
    const descriptionElement = $('#dayday');
    const cardsElement = $('#cardHolderDivGo');
    if (descriptionElement && userData.users) {
      let userBox = '';
      let cardsBox = '';
      let name, surname
      ...
      userData.users.forEach(u =>{
        // Model: PeopleUser (name, surname, email, phone_suffix, phone, gender, people_id ...)
        name       = u.name ?? splitName(u.fullname).name;
        surname    = u.surname ?? splitName(u.fullname).surname;
        // const genderText = formatGender(u.gender);
        // const email      = u.email ?? "-";
        // const phoneText  = (u.phone_suffix && u.phone) ? `+${u.phone_suffix} ${u.phone}` : (u.phoneNumber ?? u.phone ?? "-");
        const userId     = u._id ?? u.id ?? "";
        ...
        userBox += `
          <div class="flex pb-3">
            <input type="hidden" id="hiddenUserId" name="user_id[]" value="${userId}" />
            <span class="font-medium">${name} ${surname}</span>
          </div>
        `;
      })

    if(userData.cards && userData.cards.length > 0){
      userData.cards.forEach(car => {
        cardsBox += `
        <div class="flex items-center justify-between w-full">
            <span class="dark:text-white opacity-50 text-messages text-[12px]">
                ${car.name}:
      ...
      descriptionElement.html(userBox);
      cardsElement.html(cardsBox);
      ...
      success: function (response) {
        if (response.success) {
          if (response.otpRequired) {
            ...
            $(".emaildogrulama-div button[type='submit']")
              .off("click")
              .on("click", function (e) {
                e.preventDefault();
                confirmOtp();
            ...
            $(".emaildogrulama-div button[type='button']")
              .off("click")
              .on("click", function (e) {
                e.preventDefault();
                closeOtpPopup();
            ...
            alertModal("İşçi uğurla silindi");
          ...
          alertModal("Xəta baş verdi: " + (response.message || "Bilinməyən xəta"), 'error');
        }
      },
      error: function (xhr, status, error) {
        alertModal("İşçi silinərkən xəta baş verdi", 'error');
      },
    });
  };
  ...
  window.confirmOtp = function () {
    ...
    if (!allFilled) {
      alertModal("6 rəqəmli OTP kodunu daxil edin", 'error');
      return;
    }
    ...
    console.log("🔍 [Frontend] Sending OTP request:", requestData);
    ...
    $.ajax({
      ...
      success: function (response) {
        console.log("✅ [Frontend] OTP success response:", response);
        if (response.success) {
          alertModal("İşçi uğurla silindi");
          closeOtpPopup();
          // Refresh the table
          ...
          alertModal("Xəta baş verdi: " + (response.message || "Yanlış OTP kodu"), 'error');
        }
      },
      error: function (xhr, status, error) {
        console.log("❌ [Frontend] OTP error response:", xhr.responseText);
        alertModal("OTP təsdiqi zamanı xəta baş verdi", 'error');
      },
    });
  };
  ...
  window.editEmployee = function (objectId) {
    ...
    alertModal("Edit funksiyası hazırlanır...");
  };
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.data && result.data.length > 0) {
            ...
            alertModal(`${result.data.length} işçi tapıldı və cədvələ əlavə edildi`);
          } else {
            alertModal("Excel faylında məlumat tapılmadı", 'error');
          }
        } else {
          alertModal(result.message || "Excel yüklənərkən xəta baş verdi!", 'error');
        }
      } catch (err) {
  ...
  $("#addSelectedPeople").on("click", function () {
    ...
    if (selectedRows.length === 0) {
      alertModal("Zəhmət olmasa əlavə etmək istədiyiniz işçiləri seçin", 'error');
      return;
    }
  ...
  async function addPeopleToCompany(peopleData) {
    ...
    try {
      ...
      if (result.success) {
        alertModal(`${peopleData.length} işçi uğurla şirkətə əlavə edildi`);
        ...
        alertModal(result.message || "İşçilər əlavə edilərkən xəta baş verdi", 'error');
      }
    } catch (err) {
      alertModal("Server ilə əlaqə xətası", 'error');
    }
  ...
  window.exportSelectedToExcel = async function () {
    ...
    try {
      ...
      if (currentMainTab === "emekdaslar") {
        ...
        checkboxes = document.querySelectorAll(
          '#myTableImtiyaz tbody input[type="checkbox"]:checked'
        );
      } else if (currentMainTab === "vezife") {
        checkboxes = document.querySelectorAll(
          '#myTableVezife tbody input[type="checkbox"]:checked'
        );
      }
      ...
      if (selectedIds.length === 0) {
        alertModal("Heç bir item seçilməyib!", 'error');
        return;
      }
      ...
      if (exportButton) {
        ...
        exportButton.innerHTML =
          '<div class="flex items-center gap-2"><div class="animate-spin rounded-full h-4 w-4 border-b-2 border-current"></div><span class="text-[13px] font-medium">Yüklənir...</span></div>';
      ...
      if (response.ok) {
        ...
        notification.className =
          "fixed top-4 right-4 bg-green-500 text-white p-4 rounded-lg shadow-lg z-50 transform transition-all duration-300 translate-x-full";
      ...
      alertModal("Excel export zamanı xəta baş verdi. Yenidən cəhd edin.", 'error');
    } finally {
  ...
  window.exportToExcel = async function () {
    ...
    try {
      ...
      if (exportButton) {
        originalContent = exportButton.innerHTML;
        exportButton.innerHTML =
          '<div class="flex items-center gap-2"><div class="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div><span class="text-[13px] font-medium">Yüklənir...</span></div>';
        exportButton.style.pointerEvents = "none";
        exportButton.style.opacity = "0.7";
      }
      ...
      if (response.ok) {
        ...
        const notification = document.createElement("div");
        notification.className =
          "fixed top-4 right-4 bg-red-500 text-white p-4 rounded-lg shadow-lg z-50 transform transition-all duration-300 translate-x-full";
        notification.innerHTML = `
          <div class="flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// =================== Imtiyaz Qrupu — Flow + API (/rbac/add-imtiyaz) ===================
(function () {
  const popup = document.getElementById("YeniQrupPop");
  const nameInput = document.getElementById("imtiyazGroupNameInput");
  if (!popup || !nameInput) return;

  // CSRF: meta или hidden. Если у вас своя схема — просто скорректируй ниже.
  const csrf =
    document
  ...
  function ensureErrorContainers() {
    // Общая ошибка для попапа
    if (!document.getElementById("imtiyazFormError")) {
      const err = document.createElement("div");
      err.id = "imtiyazFormError";
      err.className = "alert alert-danger d-none";
      // попытаемся вставить в "тело" попапа без правки EJS
      (
        popup.querySelector(".modal-body, .popup-body, .content, .body") ||
        popup
      ).prepend(err);
    }
    // Ошибка под инпутом имени (если её нет)
    if (!popup.querySelector('[data-for="imtiyazGroupNameInput"]')) {
      const fb = document.createElement("div");
      fb.className = "invalid-feedback";
      fb.setAttribute("data-for", "imtiyazGroupNameInput");
      nameInput.parentElement?.appendChild(fb);
    }
  }
  ...
  function clearErrors() {
    ...
    const g = document.getElementById("imtiyazFormError");
    if (g) {
  ...
  function showFieldError(inputEl, msg) {
    ...
    const fb = popup.querySelector(
      `.invalid-feedback[data-for="${inputEl.id}"]`
    );
    if (fb) fb.textContent = msg || "Xəta";
  }
  ...
  function showFormError(msg) {
    ...
    if (g) {
      ...
      g.textContent = msg || "Xəta baş verdi.";
    }
  }

  // --- Сбор выбранных пользователей из таблицы выбора (чекбоксы в #myTablePop) ---
  function collectSelectedUserIds() {
    const scope = popup.querySelector("#myTablePop") || popup;
    const checks = scope.querySelectorAll(
      'input[type="checkbox"].js-user-checkbox:checked, input[type="checkbox"][data-id]:checked'
    );
    const out = [];
    checks.forEach((ch) => {
      const id = ch.dataset.id || ch.value;
      if (id) out.push(id);
    });
    // уникализируем
    return [...new Set(out)];
  ...
  function validateName() {
    ...
    if (!name || name.length < 2) {
      showFieldError(nameInput, "Ad ən azı 2 simvol olmalıdır.");
      return null;
    }
    if (name.length > 80) {
      showFieldError(nameInput, "Ad 80 simvoldan çox ola bilməz.");
      return null;
    }
    return name;
  }

  // Note: postAddImtiyaz removed - creation is now handled in popU.js to avoid duplicates

  function updateImtiyazTableRealtime(created) {
    console.log("updateImtiyazTableRealtime called with:", created);

    // Ensure we're on the imtiyaz tab
    if (currentMainTab !== "imtiyaz") {
      console.log("Not on imtiyaz tab, skipping table update");
      return;
    }

    let dt;
    try {
      if (window.imtiyazTable && typeof window.imtiyazTable.ajax === "object") {
        dt = window.imtiyazTable;
        console.log("Using existing imtiyazTable");
      } else {
        console.log("Trying to get DataTable instance from DOM");
        const table = $("#myTableImtiyaz");
        if (table.length && $.fn.DataTable.isDataTable(table[0])) {
          dt = table.DataTable();
          console.log("Got DataTable from DOM");
        } else {
          console.log("No DataTable found, will reinitialize");
          // Force table reinitialization
          setTimeout(() => {
            if (typeof initializeImtiyazTable === "function") {
              console.log("Calling initializeImtiyazTable");
              initializeImtiyazTable();
            } else if (typeof window.refreshImtiyazTable === "function") {
              console.log("Calling refreshImtiyazTable");
              window.refreshImtiyazTable();
            } else {
              console.log(
                "No table initialization function found, reloading page"
              );
      ...
      // Force table refresh as fallback
      setTimeout(() => {
    ...
    if (dt && dt.ajax && typeof dt.ajax.reload === "function") {
      ...
      dt.ajax.reload(function (json) {
        console.log("Imtiyaz table reloaded successfully:", json);
        if (!json || !json.data || json.data.length === 0) {
          console.warn("No data returned from server after imtiyaz creation");
        }

        // Scroll to the new row if possible
        setTimeout(() => {
          const rows = dt.rows().data();
          for (let i = 0; i < rows.length; i++) {
            if (
              rows[i].groupName === created?.name ||
              rows[i].name === created?.name
            ) {
              const rowNode = dt.row(i).node();
              if (rowNode) {
    ...
    // Fallback: manually add row if ajax reload is not available
    console.log("Adding row manually to imtiyaz table");
    ...
    const row = {
      ...
      groupName: created?.name || created?.data?.name || "Yeni Qrup",
      memberCount: String(
        created?.data?.memberCount || created?.usersAdded || 0
      ),
      createdDate: created?.data?.createdDate || currentTime,
    };

    try {
      const newRow = dt.row.add(row).draw(false);
      console.log("Row added successfully to imtiyaz table:", row);
      ...
      setTimeout(() => {
        ...
        if (rowNode) {
          $(rowNode).addClass("highlight-new-row");
          rowNode.scrollIntoView({ behavior: "smooth", block: "center" });
          setTimeout(() => $(rowNode).removeClass("highlight-new-row"), 3000);
        }
      }, 100);
    } catch (err) {
      console.error("Error adding row to DataTable:", err);
      // If manual row addition fails, force a complete table reload
      console.log("Attempting to force table reload...");
      setTimeout(() => {
        if (typeof initializeImtiyazTable === "function") {
          initializeImtiyazTable();
        } else if (typeof window.refreshImtiyazTable === "function") {
  ...
  window.openYeniQrupPopup = function openYeniQrupPopup() {
  ...
  window.closeYeniQrupPopup = function closeYeniQrupPopup() {
    popup.style.display = "none";
  };

  // Note: proceedImtiyazGroup is handled in popUp.js - don't duplicate it here

  // This function will be called by popU.js after successfully creating an imtiyaz group
  window.onImtiyazGroupCreated = function (groupData) {
    console.log("onImtiyazGroupCreated called with:", groupData);

    // Check current tab using DOM elements instead of currentMainTab variable
    const imtiyazTab = document.getElementById("imtiyazTab");
    const isImtiyazTabActive =
      imtiyazTab && imtiyazTab.classList.contains("bg-inverse-on-surface");

    // Switch to imtiyaz tab first if not already there
    if (!isImtiyazTabActive) {
      console.log("Switching to imtiyaz tab to show new group");
      if (typeof window.switchTab === "function") {
        window.switchTab("imtiyaz");
      }
    }

    // Refresh the imtiyaz table to show the new group
    setTimeout(() => {
      console.log("Refreshing imtiyaz table to show new group");

      // Simple approach: always switch to imtiyaz tab and refresh
      if (typeof window.switchTab === "function") {
        window.switchTab("imtiyaz");
      }
      ...
      setTimeout(() => {
        if (typeof window.refreshImtiyazTable === "function") {
          ...
          // Fallback: try to reinitialize the table
          try {
            if (typeof initializeImtiyazTable === "function") {
              initializeImtiyazTable();
            }
          } catch (err) {
            console.error("Error reinitializing imtiyaz table:", err);
    ...
    if (typeof showNotification === "function") {
      alertModal(
        "İmtiyaz qrupu uğurla yaradıldı: " + groupData.name,
        "success"
      );
    } else if (typeof Toastify !== "undefined") {
      Toastify({
        text: "İmtiyaz qrupu uğurla yaradıldı: " + groupData.name,
        duration: 3000,
        gravity: "top",
        position: "right",
      ...
      alertModal("İmtiyaz qrupu uğurla yaradıldı: " + groupData.name);
    }
  };

  // Note: saveImtiyazGroup is now handled in popU.js to avoid duplicate creation
  // The onImtiyazGroupCreated callback above will be called after successful creation
})();
