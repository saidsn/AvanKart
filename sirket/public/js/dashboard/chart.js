...
function loadKartlarMeblegCharts() {
  ...
  const yearsInput = kartlerMeblegYearsForm
    .find("input[name='years[]']:checked")
    .map(function () {
      return $(this).val();
    })
    .get();
  const cardsInput = kartlerMeblegCardsForm
    .find("input[name='cards[]']:checked")
    .map(function () {
      return $(this).val();
    })
    .get();
...
function setActiveTab(clickedTab, chartType) {
  ...
  allTabs.forEach((tab) => {
    ...
    tab.classList.add(
      ...
      "hover:text-messages",
      "dark:hover:text-primary-text-color-dark"
    );
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  allTabs.forEach((tab) => {
    tab.addEventListener("click", function () {
      ...
      const chartType = this.closest(".w-\\[50\\%\\]")
        .querySelector('[class*="text-[15px]"]')
        .textContent.trim();
