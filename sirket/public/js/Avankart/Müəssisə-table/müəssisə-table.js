$(document).ready(function () {
  ...
  var table = $("#myTable").DataTable({
    ...
    columns: [
        ...
        data: function (row) {
          return `<div class="flex items-center gap-2.5">
                      <div class="bg-table-hover rounded-[50%]">
                          <div >
                              <img src="${row.logo}" class="w-[50px] h-[33px] " alt="Logo">
                          </div>
                      </div>
                      <div class="w-full">
      ...
      {
        data: function (row) {
          return `<span class="text-[13px]  text-messages dark:text-primary-text-color-dark  flex justify-center font-normal">${row.invoice}</span>`;
        },
      },
      {
        data: function (row) {
          return `<span class="text-[13px]  text-messages dark:text-primary-text-color-dark  flex justify-center font-normal">${row.TransactionNumber}</span>`;
        },
      },
      ...
      {
        data: function (row) {
          return `<span class="text-[13px] text-messages dark:text-primary-text-color-dark font-normal">${row.finalAmount} ₼</span>`;
        },
      },
      {
        data: function (row) {
    ...
    initComplete: function () {
      ...
      $("#myTable thead th.filtering").each(function () {
        $(this).html(
          '<div class="custom-header flex w-fit gap-2.5 items-center"><div>' +
            $(this).text() +
