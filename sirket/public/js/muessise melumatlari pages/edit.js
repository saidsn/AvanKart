...
//   if (!toggleBtn) console.warn("toggleBtn tapılmadı!");
//   if (!dropdown) console.warn("dropdown tapılmadı!");
//   if (!selected) console.warn("selected tapılmadı!");

//   if (!toggleBtn || !dropdown || !selected) {
//     return;
//   }
...
function fillRestaurantPopup(data) {
  ...
  const uniquePhones = Array.from(
    new Set(phoneList.map((p) => `${p.prefix} ${p.number}`))
  );
  ...
  days.forEach((day, index) => {
    ...
    const isOpen = schedule[day] ? true : false;
      ...
      <div class="flex gap-1 flex-wrap">
          <div class="flex gap-2 flex-wrap">
              <div class="relative group">
                <div
                  class="w-[34px] h-[34px] rounded-full flex items-center justify-center text-xs ${isOpen ? "bg-focus text-on-primary" : "bg-stroke text-tertiary-text"}">
                  ${day} 
                </div>
                <div
                  class="${isOpen ? "" : "hidden"} absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-max px-2 py-1 text-xs text-on-primary bg-messages rounded opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  ${isOpen ? workHours : "Bağlıdır"}
                    <div
  ...
  if (!data.cards || data.cards.length === 0) {
    cardsContainer.append(`<span>Kart yoxdur</span>`);
  } else {
    data.cards.forEach((card) => {
      const cardHtml = `
      <div style="background-color: ${card.background_color}" 
           class="w-[84px] h-[27px] flex gap-1 items-center justify-center rounded-full">
        <img class="w-[16px] h-[16px]" src="https://api.avankart.com/v1/icon/${card.icon}" alt="${card.name}" />
  ...
  const userName = data.user_id
    ? `${data.user_id.name} ${data.user_id.surname}`.trim()
    : "Ad yoxdur";
...
$(document).ready(function () {
  $(document).on("submit", "#vezifeDarkForm", function (e) {
    e.preventDefault();
    ...
    if (!name) {
      alert("Vəzifə adı boş ola bilməz!");
      return;
    }
    ...
    $.ajax({
      ...
      success: function (res) {
        console.log("Yadda saxlanıldı:", res);

        $("#YeniVezifePop").addClass("hidden");
...
//Update duty
$(document).ready(function () {
  $(document).on("click", "#RedakteEtPopUpVezife .bg-primary", function (e) {
    e.preventDefault();

    const $form = $("#roleNameUpdateForm");
    const url = $form.data("url");
    const csrfToken = $form.find("input[name=_csrf]").val();
    ...
    $.ajax({
      ...
      success: function (res) {
        console.log("Redaktə edildi:", res);

        $("#RedakteEtPopUpVezife").addClass("hidden");
