function submitForm(formadi) {
  ...
  const ajaxOptions = {
    ...
    success: (response) => {
      ...
      window.otpEmail = response.email;
      window.tempDeleteId = response.tempDeleteId;
      if (response.success) {
        if (response.qr) {
        ...
        if (response.otpRequired && response.tempDeleteId) {
          const options = {};
          if (response.email) {
            if (response.acceptUrl) options.url = response.acceptUrl;
            Otp(response.email, response.tempDeleteId, options);
    ...
    error: (xhr) => {
      ...
      const errorMessage = error || errors[0] || "Bazı alanlar eksik veya hatalı.";
